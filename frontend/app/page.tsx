...
export default function LuxuryLandingPage() {
  ...
  // Hydrate theme preferences smoothly on initial render
  useEffect(() => {
  ...
  // Handle explicit toggle actions
  const toggleTheme = () => {
    if (darkMode) {
      document.documentElement.classList.remove('dark');
      ...
      <header className="sticky top-0 z-50 backdrop-blur-xl bg-white/70 dark:bg-[#0A0A0A]/70 border-b border-gray-100 dark:border-neutral-900 px-6 md:px-16 py-5 flex items-center justify-between transition-all">
        ...
        <div className="flex items-center gap-4 md:gap-8">
          <nav className="hidden md:flex items-center gap-8 text-xs font-bold uppercase tracking-widest text-gray-500 dark:text-neutral-400">
            <a href="#portfolio" className="hover:text-black dark:hover:text-white transition">The Collection</a>
            <Link href="/blogs" className="hover:text-black dark:hover:text-white transition">Market Insights</Link>
          </nav>

          {/* 🌓 SMOOTH DARK/LIGHT MODE TOGGLE BUTTON */}
          <button 
            type="button"
            onClick={toggleTheme}
          ...
          <Link href="/dashboard" className="px-4 py-2 bg-black dark:bg-white text-white dark:text-black text-[11px] font-black uppercase tracking-wider rounded-lg transition shadow-sm">
            Agent Console
          </Link>
        </div>
      </header>
